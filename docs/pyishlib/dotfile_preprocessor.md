...
Preprocessor specialised for `DotFile` objects.

Extends `FilePreprocessor` to store extracted metadata back
onto the `DotFile` instance, which the dotfile applier uses
