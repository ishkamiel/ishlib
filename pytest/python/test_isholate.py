...
class TestEnsureClaudeNetwork:
    ...
    def test_creates_network_when_absent(self):
        ...
        # Two set calls: raw.dnsmasq and ipv4.firewall=false (re-applied
        # even on create so any isholate upgrade that changes them converges).
        set_cmds = [
            cmd for cmd in checked_cmds if cmd[:3] == ["incus", "network", "set"]
        ]
        assert len(set_cmds) == 2
        set_keys = {cmd[4].split("=", 1)[0] for cmd in set_cmds}
    ...
    def test_reuses_existing_network(self):
        ...
        # Both set calls still happen so config converges on isholate upgrades.
        set_cmds = [
            cmd for cmd in checked_cmds if cmd[:3] == ["incus", "network", "set"]
        ]
        assert len(set_cmds) == 2
    ...
    def test_raw_dnsmasq_value_includes_all_domains_and_ipset(self):
        ...
        raw_sets = [
            ...
            and cmd[4].startswith("raw.dnsmasq=")
        ]
        assert len(raw_sets) == 1
        value = raw_sets[0][4][len("raw.dnsmasq=") :]
...
class TestClaudeFirewallRulesInPlace:
    ...
    def test_returns_true_when_all_checks_pass(self):
        """ipset + chain + FORWARD jump + on-disk content all current -> True."""
